...
public class GraphAL
{
  ...
  ******************************************************************/
  public void printIndegrees()
  {
    ...
    for(int thisVertex = 0; thisVertex < numVertices; thisVertex++){
      //except itself's adjacent list, go through the others vertices
      //if other vertices' adjacent list has i
      //record that other vertex
      //recommend to draw the graph out on paper to verify. Answer: 2,1,1,2,2
      int inDegreeCount = 0;
      for(int theOtherVertex = 0; theOtherVertex < numVertices; theOtherVertex++){
        if(thisVertex != theOtherVertex){
          Edge curr = adjacent[theOtherVertex];
          while(curr != null) {
            if (curr.toVertex == thisVertex)
              inDegreeCount++;
            curr = curr.next;
          }
        }
      }
      System.out.println("In-degree of vertex " + thisVertex + " is: " + inDegreeCount);
    }
  ...
  ******************************************************************/
  private void recursiveTraversal( int currVertex )
  {
    ...
    Edge curr = adjacent[currVertex];
    while(curr != null) {
      if(!visited[curr.toVertex])
        recursiveTraversal(curr.toVertex);
      curr = curr.next;
    }


      //same idea, but use do-while loop
//    visited[currVertex] = true;
//    System.out.println(currVertex);
//
//    Edge curr = adjacent[currVertex];
